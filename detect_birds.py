...
model_path = "yolov8n.pt"  # You can also try 'yolov8s.pt' for better accuracy
...
fps = int(cap.get(cv2.CAP_PROP_FPS)) or 30  # fallback if 0
width, height = 640, 360  # Resize frames for performance
...
while cap.isOpened():
    ...
    for i in range(len(results.boxes.cls)):
        if int(results.boxes.cls[i]) == 14:  # bird class
            found_bird = True
...
# --- Merge and extract only unique clips ---
